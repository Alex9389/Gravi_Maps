...
G = 6.67E-11; % Universal Gravity Const (Универсальная гравитационная постоянная)
p = 2670; % Average terrain density (Средняя плотность Земли)
Eotvos = 1E-9; %use to convert units to Eotvos (Этвеш = 10^-9 с^-2)

% Загрузка карты рельефа и получение из неё данных
...
%для алгоритма (увеличиваем размер карты)
lat  = [(start_lat_grad -cut_map*step_map_sec/3600)...
        (start_lat_grad +(size_map_lat -cut_map)*step_map_sec/3600)];
long = [(start_long_grad-cut_map*step_map_sec/3600)...
        (start_long_grad+(size_map_long-cut_map)*step_map_sec/3600)];
...
Z = load_map(start_lat_grad-cut_map*step_map_sec/3600,...
            start_long_grad-cut_map*step_map_sec/3600,...
			size_map_lat, size_map_long, step_map_sec);
...
fid = fopen(['..\..\MAPS\GradientMaps\' file_data_of_map],'w'); 
fprintf (fid ,'%d %d %d %10.6f %10.6f %10.6f %10.6f \n',size_map_lat_r,...
         size_map_long_r, step_map_sec, lat_r, long_r);
fclose (fid);

%% Computer Earth 's Radius at central grid point
a = 6378137.0; % earth semimajor axis in meters
f = 1/298.257223563; % reciprocal flattening - сжатие сферы вдоль диаметра

e2 = 2*f-f^2; % Эксцентриситет в квадрате
lat_middle  = (min(lat)+max(lat))/2; % поиск центральной точки загруженного участка
lat_middle2 = atand((1-e2)*tand(lat_middle)); % перевод геодезической широты в геоцентрическую
long_middle = (min(long)+max(long))/2; % поиск центральной точки загруженного участка
N = a/sqrt(1-e2*sin(lat_middle2)^2);
% Перевод в декартову СК (м)
X_ECEF = N*cosd(lat_middle2)*cosd(long_middle); %ошибка! (cos(long_middle))
Y_ECEF = N*cosd(lat_middle2)*sind(long_middle); %ошибка! (cos(lat_middle2)*sin(long_middle))
Z_ECEF = (N*(1-e2))*sind(lat_middle2);          %ошибка! (sin(lat_middle2))

% Расчет радиуса Земли в центре загруженного участка карты
R1 = sqrt(X_ECEF^2+Y_ECEF^2+Z_ECEF^2); 
...
y_int  = R1*(pi/180)*(step_map_sec/3600); %3 arc sec spacing 
x_int  = cosd(lat_middle)*y_int;

% Формируем сетку в м.
Y1 = 0:y_int:y_dist;
X1 = 0:x_int:x_dist;
% Проверка! почему-то периодически выходит за границы, возможно из-за округления 
if (length(Y1)>size_map_lat) || (length(X1)>size_map_long)  
  Y1 = y_int: y_int : y_dist;
  X1 = x_int: x_int : x_dist;
end 
%[X2,Y2]= meshgrid(X1,Y1); %пока не вижу смысла в ней, может пригодиться для интерполяции
...
sig =0;
% perform FFT (see Jekeli & Zhu (17), "ctr" may be +inf )
% НУЖНО ПОДУМАТЬ над необходимой степенью ctr!!!
for ctr =1:20
...
mu_zz =  ((2* pi)^2).* f_p.^2;
% see Jekeli & Zhu (21)
in_exp_sum = exp(-2* pi*alt.* f_p).* sig;
piGp2 = 2*pi*p*G;
Txx_parker = piGp2.* ifft2(mu_xx.* in_exp_sum);
Txy_parker = piGp2.* ifft2(mu_xy.* in_exp_sum);
Txz_parker = piGp2.* ifft2(mu_xz.* in_exp_sum);
Tyy_parker = piGp2.* ifft2(mu_yy.* in_exp_sum);
Tyz_parker = piGp2.* ifft2(mu_yz.* in_exp_sum);
Tzz_parker = piGp2.* ifft2(mu_zz.* in_exp_sum);
% Перевод в Этвеш
...
phi_step    = 30/60; % EGM96 provides a 30 arcmin resolution (read page 46)
lambda_step = 30/60; % EGM96 provides a 30 arcmin resolution 
% ЗАПЛАТКА +step_map_sec/3600 - т.к. выходит за границы массива
% setup array for griddata function
X_EGM96 = lambda_west+step_map_sec/3600 : step_map_sec/3600 : lambda_east; 
Y_EGM96 = phi_south  +step_map_sec/3600 : step_map_sec/3600 : phi_north  ;
[X2_EGM96,Y2_EGM96]= meshgrid(X_EGM96,Y_EGM96); 
%!!! Read spherical harmonic model %
[Nmax,Ae,GM,C,S,dC,dS] = Modelread('..\..\MAPS\geocoeff.txt'); 
[a,b,c,d,g,h,beta,psi,mu,eta] = coefficients(Nmax+3); % calculate Legendre coeff-s.            
CN = Normal(GM,Ae,Nmax);
for i = 3:1:11 %ПОЧЕМУ 11???
  C(i,1) = C(i,1) - CN(i); % Generation of the Potential Anomaly                              
end

filename ='EGM96Gradients'; % Создание файла с информацией о нормальном градиенте по EGM96
...
for phi = phi_south : phi_step : phi_north
  ...
  for lambda = lambda_west : lambda_step : lambda_east
    ...
    % Computation of geocentric distance
    ...
    for n=3:Nmax+1
      for m=1:n
        ...
        sum1 = sum1 + AA*CS *(a(n,abs(m))*PP+b(n,abs(m))*pnm(n,abs(m))+...
    ...
    fprintf (fid ,'%g %g %e %e %e %e %e %e \n',phigeodetic,...
	         lambda*180/pi ,Txx ,-(Txx+Tzz), Tzz, Txy,Txz,Tyz );
  end % of lambda
end % of phi
fclose (fid);
U = load(filename);
...
Txx_EGM96 = griddata (U(:,2), U(:,1), U(:,3), X2_EGM96, Y2_EGM96, 'v4'); % Заменить 'v4' на 'cubic'
...
% градиента, потому их обрезаем %abs УБРАТЬ
